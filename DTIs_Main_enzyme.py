...
def main():
    ...
    # 1-Random Forest
    rf = RandomForestClassifier(n_estimators=300 ,n_jobs=10,random_state= 55, class_weight='balanced', criterion='gini') 
    ...
    ab = AdaBoostClassifier(DecisionTreeClassifier(criterion='gini', splitter='best', max_depth=15, min_samples_split=2,
                        ...
                        class_weight= 'balanced'), algorithm="SAMME", n_estimators=200,random_state=32)
    ...
    ### Build feature vector and class labels
        ...
        # featureVector and labels for all pairs
        XX = np.asarray(features)
        YY = np.array(class_labels)
        utils.outputCSVfile('featurenzyme.csv',XX)
