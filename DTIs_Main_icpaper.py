...
def main():
    ...
    # 1-Random Forest
    rf = RandomForestClassifier(n_estimators=90,min_samples_split= 16, max_features= 0.2721759858906242 ,n_jobs=10,random_state= 65, class_weight='balanced', criterion='gini') 
    # 2-Neural Network
    NN = MLPClassifier(activation='relu', solver='lbfgs', alpha=1e-5, hidden_layer_sizes=(10, 2), random_state=1)

    # 3-Adaboost classifier
    ab = AdaBoostClassifier(DecisionTreeClassifier(criterion='gini', splitter='best', max_depth=6, min_samples_split=2,
                        min_samples_leaf=1, max_features=None, random_state=1,
                        max_leaf_nodes=None, min_impurity_split=None), algorithm="SAMME", n_estimators=90, random_state=32)
    ...
    target=np.loadtxt("./Input/ic/target.txt", delimiter='\t',dtype=np.float64)

    drug=np.loadtxt("./Input/ic/drug.txt", delimiter='\t',dtype=np.float64)
    maxiter = 20
    restartProb = 0.50
#       
    cos_simDD = Cosine_Similarity(drug)
    cos_simTT = Cosine_Similarity(target)
    ...
    dQ,pd=RWR.diffusionRWR(cos_simDD,maxiter,restartProb)
    td,pt=RWR.diffusionRWR(cos_simTT,maxiter,restartProb)
#    print(np.shape(pd))
#    print(np.shape(pt))
#
#    data1=DAE(np.array(pd),210,20,16,1,100,[100])
#    np.savetxt('drug_dae_d100.txt',data1)
#
#    data2=DAE(np.array(pt),204,20,32,1,150,[150])
#    np.savetxt('protein_dae_d400.txt',data2)
#    
...
#        a = []
#        for i in fused_simDr:  
#            a.append(np.asarray(i))
#        print(np.shape(a))
        fused_simTr = SNF(TTsim,K=5,t=3,alpha=1.0)
#        b = []
#        for i in fused_simTr:  
#            b.append(np.asarray(i))
#        print(np.shape(b))
#        drug=DAE(np.array(a),210,20,16,1,100,[100])
#        np.savetxt('drug1_dae_d100.txt',drug)
#
#        target=DAE(np.array(b),204,20,32,1,150,[150])
#        np.savetxt('protein_dae_d150.txt',target)
        ##^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
...
#        
    ...
    #============================================================================== 
        ...
        sumDDD2, maxDDD2 = DDD_TTT_sim(np.array(pd))
        sumTTT2, maxTTT2= DDD_TTT_sim(np.array(pt))
        
        sumDDT2,maxDDT2 = metaPath_Dsim_DT(pd,DrTr_train,2,True) 
        sumDTT2,maxDTT2 = metaPath_DT_Tsim(pt,DrTr_train,2,True)
        ...
        sumDDTT2,maxDDTT2 = metaPath_DDTT(DrTr_train,np.array(pd),np.array(pt), True)
    #============================================================================== 
...
#                            # path scores from G2
        ...
        for i in range(len(DT_score)):
            ...
            DT_score[i][14], DT_score[i][15],DT_score[i][16],DT_score[i][17],\
        ...
        utils.outputCSVfile('featureicnew.csv',XX)
        ...
        # fit the model 
#        a=estimator_function.fit(X_res, y_res)
        classifier.fit(X_res, y_res)
#        lgb_train = lightgbm.Dataset(X_res, y_res)
#        model = lightgbm.train(parameters,lgb_train)
##       
#        
#
#
#
#        #estimator_function.fit(X_res, y_res)
        predictedClass = classifier.predict(X_test_transform)
        predictedScore = classifier.predict_proba(X_test_transform)[:, 1]
#        predictedScore = predictedClass
#        predictedClass = (predictedClass> 0.5)
#        rf.fit(X_res, y_res)
...
#        predictedScore = rf.predict_proba(X_test_transform)[:, 1]
        ...
        #m=matthews_corrcoef(labels_test,y_pred)
        ...
        for i in  predictedScore:
            fpr,tpr,thresholds= roc_curve(Y[test_index],predictedClass)
            i+=1
        tp.append(tpr);
        fp.append(fpr);
        ...
        s.append(mean_squared_error(Y[test_index],predictedClass))
        m.append(matthews_corrcoef(Y[test_index],predictedClass))
        acc.append(accuracy_score(Y[test_index],predictedClass) )
        Pre.append(p.mean())
        Rec.append(r.mean())
...
#    #--------------------------------------------------------------------
    ...
    plt.show()
    disp.ax_.set_title("confusion matrix for PROPOSED")
    plt.show()
    ...
    print(ConfMx)
    print("p=", np.array( Pre).mean())
    print("r=",np.array( Rec).mean())
    print("tp=",np.array( tp).mean())
    print("fp=",np.array( fp).mean())
    print("mean_squared_error:"+ str( np.array(s).sum()))
    print("matthews_corrcoef:"+ str( np.array(m).sum()))
    print("the accuracy="+ str( np.array(acc).mean().round(decimals=3)))
