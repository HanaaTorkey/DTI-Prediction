...
def main():
    ...
    for train_index, test_index in  skf.split(X,Y):
        ...
        #------------------------------ node2vec ------------------------------
        target=np.loadtxt("./Input/gpcr/target.txt", delimiter='\t',dtype=np.float64)

        drug=np.loadtxt("./Input/gpcr/drug.txt", delimiter='\t',dtype=np.float64)
        maxiter = 20
        restartProb = 0.50
    ...
    #============================================================================== 
        ...
        sumDDT2,maxDDT2 = metaPath_Dsim_DT(np.array(pd),DrTr_train,2,True) 
        sumDTT2,maxDTT2 = metaPath_DT_Tsim(np.array(pt),DrTr_train,2,True)
    ...
    ### Build feature vector and class labels
        ...
        utils.outputCSVfile('featuregpcrnew.csv',XX)
...
#    novel_dt.to_csv(novel_DT_file,sep='\t', index=None)
    ...
    plt.show()
    disp.ax_.set_title("confusion matrix for DTiGEMS+")
    plt.show()
